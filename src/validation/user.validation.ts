...
const youtubeChannelUrl = z
  ...
  .refine((value) => extractChannelInfo(value) !== null, {
    message:
      "must be a valid YouTube channel URL (youtube.com/@handle, /channel/ID, /c/name or /user/name)",
  });
...
export const onboardingSchema = userFields.refine(
  (data) =>
    Boolean(data.userName) ||
    (Boolean(data.niche) && Boolean(data.targetAudience)),
  {
    message:
      "Provide a YouTube channel URL, or both a niche and a target audience",
    path: ["userName"],
  }
);
